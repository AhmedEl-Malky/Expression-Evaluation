/**
 * @author Ahmed Elmalky -> GitHub : Ahmed Ramadan El_Malky
 */
 ...
 */
fun isOperator(op:Char):Boolean = op=='+'||op=='-'||op=='/'||op=='*'
...
var precedences = mapOf('*' to 2 , '/' to 2 , '+' to 1 , '-' to 1) // a map that map precedence for its operator
/**
    ...
    if (infixExpression != null) {
        for(i in infixExpression){
            if (i==' ' || i==',')
                ...
                posfixExpression+=' '
                while(!conversionStack.isEmpty() && hasHigherPrecedence(conversionStack.peek(),i)){
 ...
 */
fun expressionEvaluation(postfixExpression:String?):Double?{
    var buffer=""
...
// endregion
fun main() {
    print("Enter your expression e.g.(4*5-6+10/2):")
    var expression = readlnOrNull()
    var postfixExpression = infixToPostfix(expression)
    println(postfixExpression)
    println("Result = ${expressionEvaluation(postfixExpression)}")
